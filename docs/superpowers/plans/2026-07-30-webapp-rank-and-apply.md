...
- These six suites must pass at every commit with **no assertion changed**: `test/claudeCli.test.ts`, `test/runStore.test.ts`, `test/scrapeProgress.test.ts`, `test/digestHtml.test.ts`, `test/emlBuilder.test.ts`, `test/seenDiff.test.ts`. If an assertion or an expected value needs editing to pass, the refactor changed behaviour — stop and reconsider. **Ruling (2026-07-30, user):** purely additive fixture edits are permitted — Task 6 makes new `TimelineRow` fields required, so `job()`-style fixture literals in `digestHtml.test.ts` and `reportHtml.test.ts` must gain those fields. Adding a field to a fixture is not changing an assertion.
