...
Generate a self-contained HTML dashboard from `profiles/<profile>/job_search_tracker.csv` and the application archives under `profiles/<profile>/documents/applications/`. The output is a single `.html` file — no server, no dependencies — that can be opened directly in a browser.
...
- No argument → output to `profiles/<profile>/reports/application-dashboard.html`
...
Create `profiles/<profile>/reports/` if it does not exist.
